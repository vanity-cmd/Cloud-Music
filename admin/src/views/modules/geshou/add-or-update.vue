<template>
	<div class="addEdit-block" :style='{"padding":"0"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 2% 16px 0","background":"none","display":"inline-block"}' class="input" v-if="type!='info'"  label="歌手" prop="geshou">
					<el-input v-model="ruleForm.geshou" placeholder="歌手" clearable  :readonly="ro.geshou"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 2% 16px 0","background":"none","display":"inline-block"}' v-else class="input" label="歌手" prop="geshou">
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `geshou/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.geshouCrossAddOrUpdateFlag = false;
						 this.parent.search();
