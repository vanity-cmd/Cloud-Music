...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"12px 10px 20px","margin":"20px 0 0 0","borderColor":"#52b646","alignItems":"center","borderRadius":"12px","flexWrap":"wrap","background":"#eff9ee","borderWidth":"4px","display":"flex","width":"100%","borderStyle":"solid dashed dotted double","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
        ...
        <el-input v-model="formSearch.songname" placeholder="歌名" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>歌手</div>
        <el-select v-model="formSearch.singer" placeholder="请选择歌手" :clearable="true">
          <el-option v-for="(item, index) in singerOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>音乐类型</div>
        <el-select v-model="formSearch.yinleleixing" placeholder="请选择音乐类型" :clearable="true">
          <el-option v-for="(item, index) in yinleleixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>专辑</div>
        <el-input v-model="formSearch.zhuanji" placeholder="专辑" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('yinlexinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(255,215,169,1) 0%, rgba(255,164,58,1) 100%)","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/yinlexinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"margin":"20px 0 20px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"10px 10px","borderColor":"#52b646","borderRadius":"8px","flexWrap":"wrap","background":"#fff","borderWidth":"4px","display":"flex","width":"100%","borderStyle":"solid double solid double","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"border":"2px solid #88d67e","cursor":"pointer","padding":"8px","boxShadow":"-1px 2px 4px rgba(0,0,0,.1)","margin":"10px","borderRadius":"8px","background":"#fff","display":"inline-block","width":"23%","position":"relative","borderStyle":"dotted dashed solid double","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				...
				<div :style='{"padding":"0","lineHeight":"30px","fontSize":"14px","color":"#333"}' class="name ">歌名:{{item.songname}}</div>
				<div :style='{"padding":"0","lineHeight":"30px","fontSize":"14px","color":"#333"}' class="name ">歌手:{{item.singer}}</div>
				<div :style='{"padding":"0","lineHeight":"30px","fontSize":"14px","color":"#333"}' class="name ">{{item.yinleleixing}}</div>
			</div>
		</div>
	...
	<div class="hot" :style='{"width":"100%","margin":"20px 0 20px","background":"none","height":"auto"}'>
	  ...
	  <div :style='{"padding":"20px","boxShadow":"0 1px 2px rgba(0,0,0,.1)","borderRadius":"0 0 12px 12px","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
	    <div v-for="item in hotList" :key="item" :style='{"cursor":"pointer","border":"2px solid #88d67e","padding":"8px","boxShadow":"-1px 2px 4px rgba(0,0,0,.1)","borderRadius":"8px","background":"none","width":"23%","borderStyle":"dotted dashed solid double","height":"auto"}' @click="toDetail(item)">
	      ...
	      <div :style='{"padding":"0 10px","lineHeight":"30px","fontSize":"16px","color":"#333"}'>{{item.songname}}</div>
	      <div :style='{"padding":"0 10px","lineHeight":"30px","fontSize":"16px","color":"#333"}'>{{item.singer}}</div>
	      <div :style='{"padding":"0 10px","lineHeight":"30px","fontSize":"16px","color":"#333"}'>{{item.yinleleixing}}</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/geshou/geshou').then(res => {
        if (res.data.code == 0) {
          this.singerOptions = res.data.data;
        }
      });
      this.$http.get('option/yinleleixing/yinleleixing').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get('yinlexinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
